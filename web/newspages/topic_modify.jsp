	...
	<%@ include file="console_element/left.html"%>
	  ...
	  <div id="opt_area">
	    ...
	    <form action="topic" method="post" onsubmit="return check()" >
	      <p>
	        <label> 主题名称 </label>
	        <input name="tname" type="text" class="opt_input" value="${param.tname}<%--<%=request.getParameter("tname")%>--%>" />
			  <span style="color: red">${requestScope.error}<%--<%=error==null?"":error%>--%></span>
	      </p>
			<input name="tid" type="hidden" value="${param.tid}<%--<%=request.getParameter("tid")%>--%>">
