...
<%--<%
    ...
    if (newsPage == null) {
        request.getRequestDispatcher("/news?action=backnews").forward(request, response);
        return;
    }
    ...
    <div id="main">
        ...
        <div id="opt_area">
            <ul class="classlist">
                ...
                <c:forEach items="${newsPage.newsList}" var="newsList" varStatus="nl">

                    <li>${newsList.ntitle}
                        <span> 作者：${newsList.nauthor}   &#160;&#160;&#160;&#160;
					<a href='news_modify.jsp'>修改</a> &#160;&#160;&#160;&#160;
					<a href='javascript:;' onclick="del(${newsList.nid})">删除</a>
				</span>
                    </li>
                    <c:if test="${(nl.count)%5 eq 0}">
                        <li class='space'></li>
                    </c:if>
                </c:forEach>
                ...
                <p align="right">
                    ...
                    <a href="news?action=backnews&currPageNo=1">首页</a>
                    <c:if test="${newsPage.currPageNo gt 1}">
                        <a href="news?action=backnews&currPageNo=${newsPage.currPageNo - 1}">上一页</a>
                    </c:if>
                    <c:if test="${newsPage.currPageNo lt newsPage.totalPageCount}">
                        <a href="news?action=backnews&currPageNo=${newsPage.currPageNo + 1}">下一页</a>
                    </c:if>
                    <a href="news?action=backnews&currPageNo=${newsPage.totalPageCount}">末页</a>
...
/*        function del(nid) {
            if(confirm("确定要删除吗？")){
                location.href="news?action=del&nid="+nid;
            }
        }
        $.ajax({
            url:"news",
            ...
            success:function(result){
                ...
                for (let i = 0; i < result.length; i++) {
                    var $li = $("<li>"+result[i].ntitle+"<span> 作者："+result[i].nauthor+" &#160;&#160;&#160;&#160;<a href='news_modify.jsp'>修改</a> &#160;&#160;&#160;&#160;\n" +
                        "<a href='javascript:;' onclick=\"del(result[i].nid)\">删除</a></span></li>");
                    $ul.append($li);
                        if ((i + 1) % 5 == 0) {
